...
// @author      			Mr. Nope
// @version     			2023-05-25
// @name								Motherless Plus
// @match       			*://*.motherless.com/*
// @description 	A kinder Motherless. Because you're worth it.
// @run-at						document_end
// @date        				2023-05-25
// @license     			MIT
// ==/UserScript==
...
setTimeout(() => {
   ...
   */
  const videoContainer = document.querySelector('#ml-main-video');
  const video = document.querySelector('#ml-main-video_html5_api');
  const distractions = [
    'header'
  ];
  const isOnVideoPage =
    /^http[s]*:\/\/(www\.)*motherless\.com\/watch\//.test(window.location.href) && !!videoContainer;
